...
class CpuUsageChart extends Component {
  ...
  handleRefresh = async chart => {
    ...
    try {
      ...
      let cpuDelta =
        response.stats.cpu_stats.cpu_usage.total_usage -
        response.stats.precpu_stats.cpu_usage.total_usage;
      let systemDelta =
        response.stats.cpu_stats.system_cpu_usage -
        response.stats.precpu_stats.system_cpu_usage;
      let cpuPercent;
      if (systemDelta > 0.0) {
        cpuPercent =
          (cpuDelta / systemDelta) *
          response.stats.cpu_stats.cpu_usage.percpu_usage.length *
          10000.0;
      }
